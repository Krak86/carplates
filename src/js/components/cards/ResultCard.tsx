...
import {
  itemFetchDataForPlate,
  addToFavoritesSync,
  removeFromFavoritesSync,
  itemFetchDataForVin,
} from "../../redux/actions";
...
export const ResultCard = (props: { item: IItem }) => {
  const state: IApplicationStates = useSelector(
    (stateInternal: AppState) => stateInternal.Item,
    shallowEqual
  );
  ...
  const favorite = Utils.isItemAlreadyAdded(
    state.favorites,
    props.item.n_reg_new
  );
  ...
  const secondary = `${props.item.n_reg_new}, ${
    regions[props.item.PartitionKey]
  }`;
  ...
  const weight = `${lang(state.lang).weight}: ${props.item.own_weight}/${
  ...
  const person = `${lang(state.lang).person}: ${
    props.item.person === "P"
      ? lang(state.lang).person_private
      : lang(state.lang).person_company
  }`;
  const dReg = `${lang(state.lang).d_reg}: ${props.item.d_reg}`;
  const operName = `${lang(state.lang).oper_name}: ${props.item.dep} (${
    props.item.dep_code
  }), ${props.item.oper_name} (${props.item.oper_code})`;
  const regAddrKoatuu = `${lang(state.lang).reg_addr_koatuu}: ${
    props.item.reg_addr_koatuu
  }`;
  const vinLabel = `${lang(state.lang).vin}: ${props.item.vin}`;
  const vin = props.item.vin;
  ...
  const handleAddToFavs = () => {
    setAnchorEl(null);
    if (
      Utils.isLimitExceeded(state.favorites.length, favoritsItemsLimit) === true
    ) {
    ...
    favorite === true
      ? dispatch(
          removeFromFavoritesSync(state.loggedIn, state.favorites, props.item)
        )
      : dispatch(
          addToFavoritesSync(state.loggedIn, state.favorites, props.item)
        );
  };
  ...
  const handleVinClick = (hashValue: string) => {
    const value = Utils.trimData(hashValue);
    const url = Utils.shapeUrlVin(URLs.getDataByVinUrl || "", value);
    ...
    <Fragment>
      <Card className={classes.card}>
        ...
        >
          ...
          <MenuItem onClick={handleAddToFavs}>
            {favorite === true
          ...
          <CardContent>
            ...
            {vin && (
              <Typography variant="body2" component="p">
                ...
                <Link to={`/${props.item.vin}`}>
                  <IconButton
                    className={classes.iconButton}
                    aria-label="search"
                    onClick={() => handleVinClick(props.item.vin)}
                  >
                    <SearchIcon />
                  </IconButton>
                </Link>
            ...
            {vin && (
              <Typography variant="body2" component="p">
                ...
                <a
                  href={`https://en.bidfax.info/?do=search&subaction=search&story=${props.item.vin}`}
                  target="_blank"
                >
                  {props.item.vin}
                </a>
            ...
            {vin && (
              <Typography variant="body2" component="p">
                ...
                <a
                  href={`https://vininspect.com/check-vin-number-and-get-the-vehicle-history-report/checkout/${props.item.vin}`}
                  target="_blank"
                >
                  {props.item.vin}
                </a>
            ...
            {vin && (
              <Typography variant="body2" component="p">
                ...
                <a
                  href={`https://vincheck.info/check/report-summary.php?vin=${props.item.vin}`}
                  target="_blank"
                >
                  {props.item.vin}
                </a>
