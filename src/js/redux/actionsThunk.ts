...
import { IApplicationStates, IItem, IServiceRespond, IVIN, IImageRecognizeResponse, IRiaCategories, IRiaSearch, IRiaAds, IRiaSearchData, IPlatesmania, ILoggedIn, IUserItem, IUserKeys, IEnvConfig } from "../models/Interfaces";
...
export const itemFetchDataForPlateThunk = (itemRequest: string, url: string): ThunkAction<void, IApplicationStates, null, Action<string>> => async (dispatch) => {
    ...
    .then((itemResponse: IServiceRespond) => {
        ...
        if (itemResponse.value && itemResponse.value.length && itemResponse.value.length > 0) {
            ...
            dispatch(actionCreators.addToItemsList({
                item: data,
...
export const itemFetchDataForVinThunk = (vinRequest: string, url: string): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const imageFetchDataThunk = (file: File, url: string): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const fetchDataForPlatesmaniaThunk = (itemRequest: string): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const fetchDataForRiaModelThunk = (itemResponse: IItem): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const fetchDataForRiaSearchThunk = (categoryValue: number, modelValue: number, brandValue: number, year: string, key: string): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const fetchDataForRiaAdsThunk = (key: string, ads: IRiaSearchData[]): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const authoriseUserThunk = (authStatus: ILoggedIn, favorites: IItem[]): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
...
export const updateUserThunk = (userKeys: IUserKeys, items: IItem[]): ThunkAction<void, IApplicationStates, null, Action<string>> => (dispatch) => {
