...
import { IVIN, IServiceRespond, IImageRecognizeResponse, IEnvConfig, IPlatesmania, IItem, IRiaCategories, IRiaSearch, IRiaSearchData, IRiaAds } from "../models/Interfaces";
...
export function* watchNewRequestFetchDataForPlateStart() {
    yield takeLatest(actionTypes.ITEM_FETCH_DATA_FOR_PLATE, requestFetchDataForPlate);
}
...
function* requestFetchDataForPlate(action: actionTypes.IItemFetchDataForPlate) {
    try {
        ...
        const itemResponse: IServiceRespond = yield call(UtilsAsync.fetchDataForPlateApi, action.payload.url);
...
function* requestFetchDataForVin(action: actionTypes.IItemFetchDataForVin) {
    try {
        ...
        yield put(actionCreators.itemIsLoading(true));
        yield put(actionCreators.itemHasErrored(false));
        const itemResponse: IVIN = yield call(UtilsAsync.fetchDataForVinApi, action.payload.url);
        yield put(actionCreators.itemIsLoading(false));
...
function* requestImageFetchData(action: actionTypes.IImageFetchData) {
    try {
        ...
        yield put(actionCreators.itemIsLoading(true));
        yield put(actionCreators.itemHasErrored(false));
        const itemResponse: IImageRecognizeResponse = yield call(UtilsAsync.imageFetchDataApi, action.payload.value, action.payload.url);
        yield put(actionCreators.itemIsLoading(false));
        if (itemResponse.results && itemResponse.results.length && itemResponse.results.length > 0) {
...
function* fetchDataForPlatesmania(value: string) {
    try {
        ...
        const itemResponse: IPlatesmania = yield call(UtilsAsync.fetchDataForPlatesmaniaApi, carPlate, url);
